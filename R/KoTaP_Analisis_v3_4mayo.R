...
# ==============================================================================
# SECCIÓN 7 ─ LOS 100 TITANES VS. LA INFLACIÓN (2020–2024) - BICOLOR
# ==============================================================================
...
# 2. Base de datos (Con Traductor de Titanes para evitar Mojibake)
df_tree_base <- df |>
  ...
  summarise(ROA     = mean(ROA,  na.rm = TRUE),
            ...
            .groups = "drop") |>
  mutate(
    # Forzamos nombres occidentales para las empresas gigantes
    name_clean = case_when(
      ...
      stock == 35420  ~ "NAVER",
      stock == 35720  ~ "KAKAO",
      # Para el resto, usamos el nombre original limpiado
...
hacer_treemap_inflacion <- function(anio) {
  ...
  # Calculamos las áreas y coordenadas (Fíjate en index = "name_clean")
  tm_obj <- suppressMessages(
    treemap(df_top100, index="name_clean", vSize="activos",
  ...
  dg <- tm_obj[["tm"]] |>
    as_tibble() |>
    mutate(
      xmax = x0+w, ymax = y0+h, name_clean = as.character(name_clean),
      x_centro = x0 + (w / 2),
      y_centro = y0 + (h / 2)
    ) |>
    left_join(df_top100 |> select(name_clean, ROA_Real, activos), by = "name_clean") |>
    mutate(
      Estado     = ifelse(ROA_Real > 0, "Vence Inflación", "Pierde vs Inflación"),
      Intensidad = abs(ROA_Real),
      label_vis  = sprintf("%s\n%+.1f%%", name_clean, ROA_Real * 100)
    )
  
  p <- ggplot(dg) +
    # Color binario y transparencia basada en la intensidad del rendimiento
    geom_rect(aes(xmin=x0, ymin=y0, xmax=xmax, ymax=ymax, fill=Estado, alpha=Intensidad),
              linewidth=0.5, colour="#0F172A") +
    # Texto seguro, sin problemas de tipografía
    geom_text(aes(x = x_centro, y = y_centro, label = label_vis),
              colour="white", size=3.2, fontface="bold", lineheight=0.9, check_overlap = TRUE) +
    # Paleta tajante: Verde (Gana) y Rojo (Pierde)
    scale_fill_manual(
      values = c("Vence Inflación" = "#10B981", "Pierde vs Inflación" = "#EF4444"),
      name = "Desempeño Real"
    ) +
    # Controlamos la transparencia para que los valores mínimos no se vuelvan invisibles
    ...
    labs(
      ...
      caption  = "Cuadros más brillantes = Mayor margen de victoria/derrota frente a la inflación."
    ) +
    theme_void() +
    theme(
      legend.position = "bottom",
      legend.text     = element_text(color = "#E8EADC", size = 12),
...
# 4. Generar y guardar EXCLUSIVAMENTE los gráficos individuales
for (anio in 2020:2024) {
  p_tm <- hacer_treemap_inflacion(anio)
  if (!is.null(p_tm)) reg(p_tm, sprintf("treemap_real_bicolor_%d.png", anio))
}

cat("  Treemaps individuales completados.\n")
